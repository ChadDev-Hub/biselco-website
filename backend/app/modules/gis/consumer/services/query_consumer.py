...
async def get_consumer(session: AsyncSession, query: Optional[Any] = None):
    stmt = (select(
        func.coalesce(ST_AsGeoJSON(ConsumerMeter.geom), None).label("geolocation"),
    ...
    data = [
        Consumer(
            ...
            geolocation=(
            json.loads(result["geolocation"])
            if result.get("geolocation") and result["geolocation"] not in ("null", "")
            else None
        )
