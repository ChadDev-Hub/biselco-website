...
from sqlalchemy import Integer, Text, ForeignKey, Numeric, Boolean, Computed
...
class PrimaryLines(BaseModel):
    ...
    from_bus_id: Mapped[str] = mapped_column(
        ForeignKey("gis.bus.bus_id"), type_=Text)
    to_bus_id: Mapped[str] = mapped_column(
        ForeignKey("gis.bus.bus_id"), type_=Text)
    phasing: Mapped[str] = mapped_column(type_=Text, nullable=True)
    description: Mapped[str] = mapped_column(type_=Text, nullable=True)
    configuration: Mapped[str] = mapped_column(type_=Text, nullable=True)
    ...
    from_bus: Mapped["Bus"] = relationship(
        "Bus", foreign_keys=[from_bus_id], back_populates="pl_out_going_lines")
    to_bus: Mapped["Bus"] = relationship(
        "Bus", foreign_keys=[to_bus_id], back_populates="pl_incoming_lines")
    conductor: Mapped["ConductorWires"] = relationship(
        "ConductorWires", back_populates="primary_lines")
    neutral: Mapped["NeutralConcentricCable"] = relationship(
        "NeutralConcentricCable", back_populates="primary_lines")
    village: Mapped["Village"] = relationship(
        "Village", back_populates="primary_lines")
    municipal: Mapped["Municipality"] = relationship(
        "Municipality", back_populates="primary_lines")
