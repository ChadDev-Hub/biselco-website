...
from sqlalchemy import Text, Integer, Boolean, ForeignKey, DateTime, BIGINT, func
...
class MeterAccount(BaseModel):
    ...
    account_no:Mapped[int] = mapped_column(type_=BIGINT,unique=True, nullable=False)
    user_id:Mapped[int] = mapped_column(ForeignKey("users_account.id", ondelete="CASCADE", onupdate="CASCADE"), type_=UUID(), nullable=False)
    consumer_name: Mapped[str] = mapped_column(type_=Text, nullable=False)
    consumer_type: Mapped[str] = mapped_column(type_=Text, nullable=True)
    meter_location: Mapped[str] = mapped_column(type_=Text, nullable=False)
    mobile_no: Mapped[str] = mapped_column(type_=Text, nullable=False)
