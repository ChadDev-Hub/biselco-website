        ...
        <ul className="timeline card timeline-vertical bg-base-200 timeline-snap-icons">
            {data.map((item) => (
                ...
                <li key={item.id} className=" cursor-pointer">
                    ...
                    <div data-tip={item.description} className="timeline-middle tooltip tooltip-left  text-xs">
                        {<TimeLineSvg status_name={item.status_name} statuslist={statuslist} />}
                    </div>
                    {item.id !== data.length &&<hr className={`w-[0.2rem] ${statuslist?.includes(item.status_name) ?
                        "bg-blue-500  animate-pulse" : "bg-gray-400"}`} />}
                </li>
