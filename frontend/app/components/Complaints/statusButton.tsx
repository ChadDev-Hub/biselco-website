...
import EnableButton from '../enabledbutton'
...
const ComplaintStatusButton = ({ status, complaints_id }: Props) => {
    ...
    const handleUpdate = async (id: number, status_name: string, activated: boolean) => { 
        console.log(id, status_name, activated)
    }
    return (
        <>
            <div className="dropdown dropdown-end">
                <div aria-label='options' tabIndex={0} role="button" className="btn m-1">
                    <svg
                        fill="currentColor"
                        width={25}
                        height={25}
                ...
                <ul tabIndex={-1} className="dropdown-content menu bg-base-100 rounded-box z-1 w-xs p-2 shadow-sm">
                    {complaintStatusName.map((item: string, index: number) =>
                        <li key={index} className='flex flex-row space-y-2 items-center justify-between w-full text-md disabled'>
                            <span>{item}</span>
