  ...
  const onSubmit: SubmitHandler<FormType> = useCallback(
    async (data) => {
      ...
      switch (res.status) {
        ...
        case 201:
          reset();
          showAlert("success", "Registration Successfull Please Wait Preparing Your Ticket");
    ...
    <div className="w-full flex justify-center px-0 sm:px-32">
      ...
      >
        ...
        <section>
          ...
          {errors.name && (
            <p className="text-error text-xs italic"> {errors.name.message}</p>
          )}
        </section>
        ...
        <section>
          ...
          />
          {errors.image && (
            <p className="text-error text-xs italic"> {errors.image.message}</p>
          )}
          {/* IMAGE PREVIEW */}
