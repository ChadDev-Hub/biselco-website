...
const NewConnectionDataContainer = ({ data, searchComponent }: Props) => {
          ...
          queueMicrotask(() => {
            setNewConnectionData((prev) => {
              const existingData = prev.filter(
                (item) => item.id !== message.data.new_connection.id,
              );
              const newData = [
                message.data.new_connection,
                ...existingData,
              ].slice(0, message.number_of_features);
              return newData;
            });
          });
        } else {
  ...
  const handleDelete = async () => {
    try {
      await deleteNewConnection(selectedRow);
  ...
  const handleDownload = async (formData: object) => {
    ...
    aref.href = URL.createObjectURL(res);
      ...
      <div className="w-full flex justify-center px-2">
        ...
        <div className="navbar max-w-2xl items-center  bg-base-100 border-gray-100 shadow-md w-full  px-2 flex justify-between glass mb-2 rounded-box">
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedRow.size > 0 && (
              <div className="badge badge-outline badge-secondary text-xs font-bold">
                <span>Items: </span>
                <span>{selectedRow.size === 0 ? "" : selectedRow.size}</span>
              </div>
            )}
          </div>
          {/*Search*/}
