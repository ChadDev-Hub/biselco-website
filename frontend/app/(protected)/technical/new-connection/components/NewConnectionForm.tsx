...
type FormField = {
  ...
  realtime_image: FileList | undefined;
};
...
const NewConnectionForm = () => {
  ...
  const realTimeImage = useWatch({ control, name: "realtime_image" });

  const imageAttachment = realTimeImage?.[0] ?? attachment?.[0] ?? undefined;
  ...
  useEffect(() => {
    ...
    const runCheck = async () => {
      ...
      if (!latitude || !longitude) {
        setError("lat", {
          message:
            "Unable to verify location of image attached please pin the exact location",
        });
  ...
  useEffect(() => {
    if (realTimeImage?.length === 0 || realTimeImage === undefined) return;
    const runCheck = async () => {
      setImageLocationVerifying(true);
    ...
    runCheck();
  }, [realTimeImage, clearErrors, setValue]);

  const onSubmit: SubmitHandler<FormField> = async () => {
    const data = getValues();
    const form = new FormData();
    Object.entries(data).forEach(([key, value]) => {
      if (key === "attachment" || key === "realtime_image") {
        return;
      }
          ...
          >
            <div>
              ...
              <div className="flex flex-col">
                <label className={labelStyle}>Date Accomplished</label>
                <input
                ...
                />
                {errors.date && (
                  <span className={errorStyle}>{errors.date.message}</span>
                )}
              </div>
              {/* CONSUMER NAME */}
              <div className="flex  flex-col">
              ...
              {/* METER SERIAL NUMBER */}
              <div className="flex flex-col">
                <label className={labelStyle}>Meter Serial Number</label>
                <input
                  {...register("meter_serial_number", {
                    required: "Please enter correct Seriral Number",
                  })}
                  type="text"
                  className={inputStle}
                ...
                {errors.meter_serial_number && (
                  <span className={errorStyle}>
                    {errors.meter_serial_number.message}
                  </span>
                )}
              </div>
              ...
              {/* METER BRAND*/}
              <div className="flex flex-col">
                <label className={labelStyle}>Meter Brand</label>
                <input
                  {...register("meter_brand", {
                    required: "Please enter meter brand",
                  })}
                  type="text"
                  className={inputStle}
              ...
              {/* METER SEALED */}
              <div className="flex flex-col">
                <label className={labelStyle}>Meter Seal</label>
                <input
                  {...register("meter_sealed")}
                  type="text"
                  className={inputStle}
                  placeholder="Meter Seal"
                />
              </div>

              {/* INITIAL READING */}
              <div className="flex flex-col">
                <label className={labelStyle}>Initial Reading</label>
                <input
                  type="number"
                  accept="number"
                ...
                {errors.initial_reading && (
                  <span className={errorStyle}>
                    {errors.initial_reading.message}
                  </span>
                )}
              </div>
              ...
              {/* ACCOMPLISHED BY */}
              <div>
                <label className={labelStyle}>Accomplished By</label>
                <input
                  {...register("accomplished_by", {
                    required: "Accomplished By is required",
                  })}
                  type="text"
                  className={inputStle}
                  placeholder="Accomplished By"
                />
                {errors.accomplished_by && (
                  <span className={errorStyle}>
                    {errors.accomplished_by.message}
                  </span>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <div className="flex gap-2 items-center">
                  <input
                    ...
                    accept="*/*"
                    type="file"
                    className="file-input file-input-sm file-input-success w-full "
                  />
                  <p>OR</p>
                  <label className="label group cursor-pointer bg-base-300 rounded-full ring text-gray-400 ring-gray-300 hover:border hover:border-dashed hover:border-emerald-500 hover:text-emerald-500  p-4">
                    <input
                      type="file"
                      className="hidden"
                      {...register("realtime_image")}
                    />
                ...
                {imageAttachment && (
                  <ImageViewer image={URL.createObjectURL(imageAttachment)} />
                )}
                {errors.attachment && (
                  <span className={errorStyle}>
              ...
              <div className="flex relative  flex-col">
                ...
                <input
                  type="hidden"
                  {...register("lon", { required: "Please Select a location" })}
                />
                <input
                  type="hidden"
                ...
                />
                {imageLocationVerifying && (
                  <p className="skeleton w-full absolute top-[50%] left-[20%] text-lg italic font-semibold skeleton-text">
                    Verifying Image Location..
                  </p>
                )}
                {errors.lat && (
                  <span className={errorStyle}>{errors.lat.message}</span>
                )}
              </div>
            </div>
