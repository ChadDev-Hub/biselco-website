...
const ChangeMeterForm = () => {
    ...
    useEffect(() => {
        ...
        fetchConsumer();
    }, [debounceSearch, router, selectedConsumer]);
    ...
    const onSubmit: SubmitHandler<FormField> = (data) => {
        ...
        SubmitChangeMeter(NewData).then((res) => {
            switch (res?.status) {
                case 201:
                    reset();
                    showLoading(false);
    ...
    return (
        <fieldset className='fieldset drop-shadow-md glass rounded-box w-full max-w-200 border"'>
            <legend className={`fieldset-legend text-3xl text-blue-800 text-shadow-md text-shadow-amber-600 ${facinate.className}`}>Change Meter Form</legend>
            <form onSubmit={handleSubmit(onSubmit)} className="flex overflow-x-auto h-full flex-col gap-2 p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col self-center items-center justify-center ">
                            ...
                            {attachment?.[0] &&
                                <Image src={attachment ? URL.createObjectURL(attachment[0]) : ""}
                                    ...
                                    className="max-h-40 mt-4 drop-shadow-2xl drop-shadow-gray-700" />
                            }
                        </div>
                    </div>
                    {/* Remarks */}
                    <div>
                        ...
                        <input title="Consumer Name" placeholder="Remarks" className="input w-full" type="text" />
                    </div>
                    {/* Accomplished By */}
                    <div>
