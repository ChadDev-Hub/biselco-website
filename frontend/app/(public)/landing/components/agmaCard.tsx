  ...
  return (
    ...
    >
      ...
      <figure className="w-full  relative aspect-square sm:aspect-auto md:aspect-auto">
        <Image
          loading="eager"
          src={data?.image_src ?? "/biselco-icon.png"}
          alt={data?.title ?? "biselco-icon"}
          fill
      ...
      <div className="card-body flex flex-col bottom-0 p-0 w-full absolute  overflow-hidden bg-transparent">
        ...
        >
          ...
          <path
            ...
            d="M0.131531 42.3431C496.072 -120.432 458.322 253.602 852.905 42.3431V291H0.131531L0.131531 42.3431Z"
          ></path>
          <path
            className="fill-base-200/50"
            d="M0 56.6734C445.7 -6.9999 498.477 210 852.774 56.6734V131C852.774 131 0.230428 71.0163 4.66999e-05 130.976C3.90831e-05 105.306 0 56.6734 0 56.6734Z"
          ></path>
        </svg>
        <div className="group z-50 w-1/2 ml-2.5 mt-50  relative">
          <div className="absolute -bottom-12">
            <div className=" text-center ">
              ...
              >
                {`"${data?.qoute_title}`.split("").map((char, index) => (
                  <motion.span key={index} variants={letterVariant}>
                    {char}
                  </motion.span>
                ))}
        ...
        <div className="card-actions flex justify-between items-end mt-8 w-full  h-full p-4  z-40">
          <div>
            <CountDown
              is_active={data?.is_active ?? false}
              targetDate={data?.target_date ?? 0}
            />
          </div>
          <div>
            {data?.is_active && (
            <Link
              href="/agma-registration"
              type="button"
              className="btn btn-primary drop-shadow-lg z-50 active:scale-105 btn-sm sm:btn-md shadow-lg"
            >
              Register Now
            </Link>
          )}
